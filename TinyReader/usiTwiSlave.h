...
//##################################################################################################
        ...
        Read Bytes from the Slave (read data from Slave txbuffer)
                ...
                2.  send - "Buffer-Address"   Address in which you like to start
with write or read.

                3.  send - "SLave-Address + read"
                4.	send - data = i2c_readAck();  Demands the Slave to
send data, start by txbuffer[Buffer-Address].

        Write Bytes to the Slave Registers (write data in to Slave rxbuffer)
                1.  send - "SLave-Address + write"
                2.  send - "Buffer-Address"   Address in which you like to start
with write or read.
                3.  send - "data"                 writes data in the buffer,
start by rxbuffer[Buffer-Address]
...
#define buffer_size 120 // 7 int values (16-bit) + optional separators

volatile uint8_t rxbuffer[buffer_size];
volatile uint8_t txbuffer[buffer_size]; // Der Sendebuffer, der vom Master
