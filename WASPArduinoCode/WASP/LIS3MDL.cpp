...
void LIS3MDL::setTimeout(uint16_t timeout) { io_timeout = timeout; }

uint16_t LIS3MDL::getTimeout() { return io_timeout; }
...
bool LIS3MDL::init(deviceType device, sa1State sa1) {
  ...
  if (device == device_auto || sa1 == sa1_auto) {
    // check for LIS3MDL if device is unidentified or was specified to be this
    // type
    if (device == device_auto || device == device_LIS3MDL) {
      // check SA1 high address unless SA1 was specified to be low
      if (sa1 != sa1_low &&
          testReg(LIS3MDL_SA1_HIGH_ADDRESS, WHO_AM_I) == LIS3MDL_WHO_ID) {
        sa1 = sa1_high;
      ...
      else if (sa1 != sa1_high &&
               testReg(LIS3MDL_SA1_LOW_ADDRESS, WHO_AM_I) == LIS3MDL_WHO_ID) {
        sa1 = sa1_low;
        if (device == device_auto) {
          device = device_LIS3MDL;
        }
    ...
    // make sure device and SA1 were successfully detected; otherwise, indicate
    // failure
    if (device == device_auto || sa1 == sa1_auto) {
      return false;
    }
  ...
  case device_LIS3MDL:
    address =
        (sa1 == sa1_high) ? LIS3MDL_SA1_HIGH_ADDRESS : LIS3MDL_SA1_LOW_ADDRESS;
    break;
  }
