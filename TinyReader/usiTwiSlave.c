/*#################################################################################################
        ...
        Created from Atmel source files for Application Note AVR312: Using the
USI Modulea
...
#elif defined(__AVR_ATmega165__) | defined(__AVR_ATmega325__) |                \
    defined(__AVR_ATmega3250__) | defined(__AVR_ATmega645__) |                 \
    defined(__AVR_ATmega6450__) | defined(__AVR_ATmega329__) |                 \
...
void usiTwiSlaveInit(uint8_t ownAddress) {
  ...
  DDR_USI |=
      (1 << PORT_USI_SCL) | (1 << PORT_USI_SDA); // Set SCL and SDA as output
  ...
  USICR =
      ...
      (1 << USIWM1) |
      (0 << USIWM0) | // set USI in Two-wire mode, no USI Counter overflow hold
      (1 << USICS1) | (0 << USICS0) |
...
ISR(USI_START_VECTOR) {
  overflowState = USI_SLAVE_CHECK_ADDRESS; // set default starting conditions
                                           // for new TWI package
  DDR_USI &= ~(1 << PORT_USI_SDA);         // set SDA as input
  ...
  if (!(PIN_USI & (1 << PIN_USI_SDA))) { // a Stop Condition did not occur
    USICR = (1 << USISIE) | // keep Start Condition Interrupt enabled to detect
                            // RESTART
            (1 << USIOIE) | // enable Overflow Interrupt
            (1 << USIWM1) | (1 << USIWM0) | // set USI in Two-wire mode, hold
                                            // SCL low on USI Counter overflow
            (1 << USICS1) | (0 << USICS0) |
            (0 << USICLK) | // 4-Bit Counter Source = external, both edges;
                            // Clock Source = External, positive edge
            (0 << USITC);   // no toggle clock-port pin
    ...
    USICR =
        ...
        (1 << USIWM1) |
        (0
         << USIWM0) | // set USI in Two-wire mode, no USI Counter overflow hold
        (1 << USICS1) |
        (0 << USICS0) | (0 << USICLK) | // 4-Bit Counter Source = external, both
                                        // edges; Clock Source = external,
                                        // positive edge
        (0 << USITC);                   // no toggle clock-port pin
  }

  USISR = (1 << USI_START_COND_INT) |
          (1 << USIOIF) | // clear interrupt flags - resetting the Start
                          // Condition Flag will release SCL
          (1 << USIPF) | (1 << USIDC) |
          (0x0 << USICNT0); // set USI to sample 8 bits (count 16 external SCL
                            // pin toggles)
}

//##################################################################################################
// ISR( USI_OVERFLOW_VECTOR )
ISR(USI_OVERFLOW_VECTOR) // Handles all the communication. Only disabled when
                         // waiting for a new Start Condition.
{
  uint8_t data = 0;
  switch (overflowState) {
  //###### Address mode: check address and send ACK (and next
  // USI_SLAVE_SEND_DATA) if OK, else reset USI
  case USI_SLAVE_CHECK_ADDRESS:
    if (USIDR == 0 || (USIDR & ~1) == slaveAddress) {
      if (USIDR & 0x01) {
        overflowState =
            USI_SLAVE_SEND_DATA; // Master Write Data Mode - Slave transmit
      } else {
        overflowState =
            USI_SLAVE_REQUEST_DATA; // Master Read Data Mode - Slave receive
  ...
  case USI_SLAVE_CHECK_REPLY_FROM_SEND_DATA:
    if (USIDR) {
      SET_USI_TO_TWI_START_CONDITION_MODE(); // if NACK, the master does not
                                             // want more data
      return;
    }

  // from here we just drop straight into USI_SLAVE_SEND_DATA if the master sent
  // an ACK
  ...
  case USI_SLAVE_GET_DATA_AND_SEND_ACK:
    ...
    {
      if (data <=
          buffer_size) // Kontrolle ob gew�nschte Adresse im erlaubten bereich
      {
